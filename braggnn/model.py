...
class BraggNN_MLP(torch.nn.Module):  
    def __init__(self, fcsz=(50, 50, 50)): 
        super().__init__()
        self.dense_ops = []
        dense_in_chs  = [121] + fcsz
        
        for ic, oc in zip(dense_in_chs, fcsz):
            self.dense_ops += [torch.nn.Linear(in_features=ic, out_features=oc, bias = True),
                            torch.nn.LeakyReLU(negative_slope=0.01), ]
        # output layer
        self.dense_ops += [torch.nn.Linear(in_features=fcsz[-1], out_features=2, bias = True), ]
        self.dense_layers = torch.nn.Sequential(*self.dense_ops)
